...
class RoomsTrans(Transition):
    ...
    def guard(self, state: Tuple[Tuple, Tuple]) -> bool:
        return self.source_mode.compute_direction(np.array(state[1])) == self.source

    def jump(self, target: str, state: Tuple[Tuple, Tuple]
             ) -> Tuple[Tuple, Tuple]:
        s = self.source_mode.change_of_coordinates(np.array(state[1]), self.source)
