...
class MyPool:
    ...
    def map(self, fn: Callable, task_args: List[Any]) -> List[Any]:
        ...
        tasks_per_process = [
            num_tasks // num_processes + 1
            if i < num_tasks % num_processes
            else num_tasks // num_processes
            for i in range(num_processes)
        ]
        assert sum(tasks_per_process) == num_tasks
        start_indices = [0 for _ in range(num_processes)]
        for i in range(1, num_processes):
            start_indices[i] = start_indices[i-1] + tasks_per_process[i-1]

        pipes = [Pipe() for _ in range(num_processes)]
        processes: List[Process] = [
            Process(
                target=run_mypool_tasks,
                kwargs=dict(
                    fn=fn,
                    args=task_args[start_indices[i] : start_indices[i] + tasks_per_process[i]],
                    pipe=pipes[i][1],
                ),
            )
            for i in range(num_processes)
