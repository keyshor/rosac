...
class MultiCategoricalPd(Pd):
    ...
    def logp(self, x):
        return tf.add_n([p.logp(px) for p, px in zip(self.categoricals,
...
class SoftMultiCategoricalPd(Pd):  # doesn't work yet
    def __init__(self, low, high, flat):
        ...
        self.categoricals = list(map(SoftCategoricalPd, tf.split(
            flat, high - low + 1, axis=len(flat.get_shape()) - 1)))
    ...
    def logp(self, x):
        return tf.add_n([p.logp(px) for p, px in zip(
            self.categoricals, tf.unstack(x - self.low, axis=len(x.get_shape()) - 1))])
