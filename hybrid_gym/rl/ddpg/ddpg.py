...
# Files
from hybrid_gym.rl.ddpg.noise import OrnsteinUhlenbeckActionNoise as OUNoise
from hybrid_gym.rl.ddpg.noise import NormalActionNoise
from hybrid_gym.rl.ddpg.replaybuffer import Buffer
from hybrid_gym.rl.ddpg.actorcritic import Actor, Critic
from hybrid_gym.rl.util import test_policy, get_rollout
...
class DDPG:
    ...
    def set_use_gpu(self):
        ...
        optimizer_to(self.actorOptim, 'cuda')
        optimizer_to(self.criticOptim, 'cuda')
        optimizer_to(self.criticOptim2, 'cuda')
        self.replayBuffer.gpu()
        self.use_gpu = True

    def set_use_cpu(self):
        self.actor.set_use_cpu()
        self.actor = self.actor.cpu()
        self.critic = self.critic.cpu()
        self.critic2 = self.critic2.cpu()
        self.targetActor.set_use_cpu()
        self.targetActor = self.targetActor.cpu()
        self.targetCritic = self.targetCritic.cpu()
        self.targetCritic2 = self.targetCritic2.cpu()
        optimizer_to(self.actorOptim, 'cpu')
        optimizer_to(self.criticOptim, 'cpu')
        optimizer_to(self.criticOptim2, 'cpu')
        self.replayBuffer.cpu()
        self.use_gpu = False
